    ...
    opts.ToolboxOpts(
        feature=opts.ToolBoxFeatureOpts(
            save_as_image=opts.ToolBoxFeatureSaveAsImageOpts(
                background_color="white", pixel_ratio=5
            ),
            restore=None,
            data_view=None,
            data_zoom=None,
...
def get_title_with_date(titlestr: str = ""):
    return opts.TitleOpts(
        title=titlestr,
        title_link="https://wkevin.github.io/szzk",
...
# 1. Drop old rank columns to prepare for recalculation
dfsort.drop(columns=years, inplace=True, errors='ignore')

# 2. Add new schools from 2025 list
new_schools = df_school_info_2025[~df_school_info_2025["代码"].isin(dfsort["代码"])]
if not new_schools.empty:
    new_schools_with_scores = pd.merge(new_schools, df_score_ac, on="代码", how="left")
    dfsort = pd.concat([dfsort, new_schools_with_scores], ignore_index=True)

# 3. Update school names and properties for all schools using the 2025 info
dfsort.set_index('代码', inplace=True)
dfsort.update(df_school_info_2025.set_index('代码'))
dfsort.reset_index(inplace=True)

# 4. Recalculate ranks for all years on the now-complete dfsort
l = len(dfsort)
for y in years:
    _l = len(dfsort[dfsort[f"[{y}]"].notnull()])
    
    if f"招生人数_{y}" in dfsort.columns.tolist():
        dfsort = dfsort.sort_values(
            by=[f"[{y}]", f"招生人数_{y}", "代码"], ascending=[False, False, True]
        )
    else:
        dfsort = dfsort.sort_values(by=[f"[{y}]", "代码"], ascending=[False, True])
    ...
    for i in range(0, _l, 1):
        zs[i] = i + 1
    dfsort.insert(len(dfsort.columns), y, zs)
...
for y in years:
    dfsort["学校名称"] += "," + dfsort[f"[{y}]"].astype(str)
...
l = (
    ...
    .set_global_opts(
        title_opts=get_title_with_date("深圳高中排名演变图"),
...
grid = Grid(
    init_opts=opts.InitOpts(
        width="100%", height="1600px", renderer="png", bg_color="white"
    )
)
