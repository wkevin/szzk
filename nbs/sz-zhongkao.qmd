...
ac_rate_2024 = 0.45  # 2023 年 AC 类报名比例
...
df_admit_number_2023 = pd.read_csv(
    f"./sz-zhongkao/admit_number_2023.csv", decimal=","
...
_df23 = transpose_ac_and_d("2023", only_gov=False, add_new_tag=False,df_input=df_admit_number_2023).dropna(subset=['AC','D','ACD'], how='all')
_df24 = transpose_ac_and_d("2024", only_gov=False, add_new_tag=False,df_input=df_admit_number_2024).dropna(subset=['AC','D','ACD'], how='all')
...
总体录取比例与 2023 年几乎持平，AC 类录取率提高，D 类持平。
...
但其实两个问题是相关的：指标生使用率降低即正录比例升高，

- 乐观解读可以是：更多的学位留个正录，靠实力上学更公平。
- 悲观解读可以是：更少的学位留个指标生，对整个深圳各区教育资源不公平的现状来说是不合理的，并且会抬高正录的分数线，感觉学校更难考了。

同一个事情，乐观和悲观角度不同，感受不同，各自解读吧。

但在绘制正录等级图时，如何合理的去掉指标生，我采取了这样的数学模型：

- 根据排序，前 30 所学校的指标生有效使用率为 1 —— 即：前 30 大的指标生不会浪费的。
- 从 31 开始逐步降低，至当年度的前 80% 院校降为 0 —— 如：2023 年 89 所公办学校，31.北莫 - 72.行知 的指标生录取率从 1 至 0 等阶下降，41 所学校，下降阶梯为 0.025
- 80% 之后，指标生录取率取 0 —— 即指标生忽略不计。
...
def level_sunburst(year: str, ac_or_d: str, enroll_total: int):
    ...
    for name, row in _df.iterrows():
        ...
        zbs_rate = (
            ...
            if _idx <= 30
                ...
                1 - (_idx - 30) / (_df23t["公办"] * 0.8 - 30)
                if _idx <= _df23t["公办"] * 0.8
                else 0
            )
        )
        _zbs *= zbs_rate  # 用录用比例修订指标生
        print(_idx, zbs_rate, _zbs, _all)
    ...
    - 指标生录用率模型修订后：指标生实录 {int(adm_sum_zbs_offset)}人, 正录实录 {int(adm_sum - adm_sum_zbs_offset)}人，
...
我们调整一下假设数据，把指标生的使用率提高 10% 再画一次：

```{python}
# | output: asis

level_sunburst("2024", "AC", enroll_number_2024_ac , zbs_rate_2024 + 0.1).render_notebook()
```
