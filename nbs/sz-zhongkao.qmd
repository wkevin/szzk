...
- 网络数据: 2023 年 126000 报名，AC 类考生约 57800 人，D 类考生约 68200 人。—— **此数据本人未能证实，不承担责任，仅供预估参考。**
- 其他数据来自深圳招生办公室公开文件。

```{python}
# | output: true
enroll_rate_table("2023", 57800, 68200, zbs_rate_2023).render_notebook()
```

- 上表为招生计划中的比例，实际并非所有指标生名额都能被用到，2022 年指标生使用比例就比较小，2023 年使用比例网传会比较高，我们按 0.5 做修订。
- 上表显示：AC 类录取比例 84%，如果指标生都用满的话，84% 中的 41% 会通过指标生上学，但 0.5 修订后变成：21% 指标生 + 63% 正录。

### 2024

- 网络数据: 2024 年 131000 报名，AC 类考生约 61000 人，D 类考生约 70000 人。—— **此数据本人未能证实，不承担责任，仅供预估参考。**
- 其他数据来自深圳招生办公室公开文件。

```{python}
# | output: true
enroll_rate_table("2024", 61000, 70000, zbs_rate_2024).render_notebook()
```

- 总体录取比例与 2023 年几乎持平。
- 指标生有效比例根据 2023 年比较低，有些新学校 25% 都不到，此处取 0.4 做修正（比 2023 的 0.5 降低一些）
...
def level_sunburst(year: str, ac_or_d: str, enroll_total: int, zbs_rate: float = 0.5):
    ...
    for name, row in _df.iterrows():
        adm_all = row[f"{ac_or_d}"] if not pd.isnull(row[f"{ac_or_d}"]) else 0
        adm_zbs = (
        ...
        adm_zbs *= zbs_rate  # 用录用比例修订指标生
        adm_zbs = int(adm_zbs)

        idx += 1
        data.append(
            {"name": str(idx) + "_" + row["学校名称"], "value": int(adm_all - adm_zbs)}
        )
        adm_sum += int(adm_all)
        adm_sum_zbs += adm_zbs

    print(
        f"{ac_or_d} 类总报名 {enroll_total}, 总录取 {int(adm_sum)}，\n{zbs_rate} 比例修订后预估：指标生录取 {adm_sum_zbs}人, 正录录取 {adm_sum - adm_sum_zbs}人，\n正录报名 {enroll_total - adm_sum_zbs}人，缺口{enroll_total - adm_sum}"
    )
    # 计算缺口时也要减去指标生
    data.append(
        {
    ...
    return (
        ...
        .set_global_opts(
            title_opts=get_title(f"{year} {ac_or_d} 类报考等位图"),
            toolbox_opts=g_toolbox_opts,
            legend_opts=opts.LegendOpts(is_show=False),
        )
    )
...
第 1 个问题因为指标生运作机制，且涉及因素较多，致使指标生通常比正录的分数要低，所以正录的分数线通常在最终高一开学后可能是中位线，而不是最低线（新学校除外，可能有特例）—— 所以此处不讨论该问题。

第 2 个问题用下面的图展示，主要描述了**招生人数**与**报名人数**的关系：

- 下面的图推荐在电脑上打开，手机上查看比较拥挤。
- 内圈 A+、A、B+、B 等是报名人数的等位
- 外圈是各个学校按前次录用最低分数线排名后的，招生人数累积值
- 外圈的缺口：即报名人数与招生人数之间的差值 —— 民办或职校去向
- 指标生会**预估**一个录用比例: 0.5 —— 即只有 50% 的指标生名额被使用，其余的名额会放回到正录
...
```{python}
# | output: true
level_sunburst("2023", "AC", 57800, zbs_rate_2023).render_notebook()
```
...
```{python}
# | output: true
level_sunburst("2023", "D", 68200, zbs_rate_2023).render_notebook()
```
...
```{python}
# | output: true
level_sunburst("2024", "AC", 61000, zbs_rate_2024).render_notebook()
```
...
```{python}
# | output: true
level_sunburst("2024", "D", 70000, zbs_rate_2024).render_notebook()
```
